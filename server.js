...
app.use(express.static(__dirname, { maxAge: isProduction ? '1d' : 0, etag: true }));
...
app.post('/api/export/excel', async (req, res) => {
  try {
    ...
    if (data.type === 'lesson') {
      ...
      t1.value = `${data.className}  ·  ${data.lessonName}`;
      t1.font = { name: 'Calibri', size: 16, bold: true, color: { argb: 'FF2C2416' } };
      t1.alignment = { vertical: 'middle', horizontal: 'left' };
      t1.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF8F0' } };
      ws.getRow(1).height = 36;

      ws.mergeCells(2, 1, 2, colCount);
      const t2 = ws.getCell('A2');
      t2.value = `Date: ${data.lessonDate}  ·  Generated ${new Date().toLocaleDateString()}`;
      t2.font = { name: 'Calibri', size: 10, italic: true, color: { argb: 'FFA08060' } };
      t2.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF8F0' } };
      ...
      t4.value = `Attendance: ${rate}%  |  Present: ${present}  Late: ${late}  Absent: ${absent}`;
      t4.font = { name: 'Calibri', size: 10, color: { argb: 'FF5C4A2A' } };
      t4.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF0DC' } };
      ...
      (data.rows || []).forEach((row, idx) => {
        ...
        (row.grades || []).forEach((grade, ci) => { exRow.getCell(ci + 3).value = grade || ''; });
      ...
      c1.value = `${data.className}  ·  Class Roster`;
      c1.font = { name: 'Calibri', size: 16, bold: true, color: { argb: 'FF2C2416' } };
      c1.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF8F0' } };
      ws.getRow(1).height = 36;

      ws.mergeCells('A2:F2');
      const c2 = ws.getCell('A2');
      c2.value = `Generated ${new Date().toLocaleDateString()}  ·  ${(data.rows || []).length} students`;
      c2.font = { name: 'Calibri', size: 10, italic: true, color: { argb: 'FFA08060' } };
      c2.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF8F0' } };
      ws.getRow(2).height = 20;
